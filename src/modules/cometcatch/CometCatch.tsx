...
export function CometCatch() {
  ...
  // Tilt steering (orientation, with motion-sensor fallback) + touch drag.
  useEffect(() => {
    if (phase !== "play") return;
    const detachTilt = attachTilt((tx) => {
      setTiltActive(true);
      sceneRef.current?.steer(Math.max(-30, Math.min(30, tx)) / 30);
    });
    ...
    <div className="screen stage-play">
      ...
      {phase === "play" && (
        <div className="catch-hud">
          ...
          {!tiltActive && seconds <= ROUND_SECONDS - 4 && (
            <span className="catch-stat orbit-note">👆 No tilt? Drag to steer!</span>
          )}
        </div>
      )}
      ...
      {phase === "intro" && (
        <div className="overlay">
          <div className="panel celebration-card">
            <h2>🌠 Comet Catch</h2>
            <p className="catch-howto">Hold the tablet comfy — then tap Start!</p>
            <p className="catch-howto dim">Tilt left and right to fly.</p>
